...
def donut_pie(ax, counts, title):
    ax.pie(
        ...
        colors=PALETTE, wedgeprops={"width": 0.4, "edgecolor": "white"},
...
def fp_prevalence_bar(col, label_map, order, title, filename, p_label="", rotate=0):
    ...
    ).rename(index=label_map).reindex(order).dropna()
...
fig, ax = plt.subplots(figsize=(5, 5))
counts = df["education"].value_counts().reindex(["None", "Primary", "Secondary", "Higher"]).dropna()
...
ax.pie(
    counts, labels=counts.index, autopct="%1.1f%%",
    colors=CONTRAST, wedgeprops={"width": 0.4, "edgecolor": "white"},
    pctdistance=0.5, labeldistance=1.05,
)
...
fig, ax = plt.subplots(figsize=(7, 4))
bars = ax.bar(list(method_pcts.keys()), list(method_pcts.values()),
...
bars = ax.bar(counts.index, counts.values / n * 100,
...
fp_prevalence_bar(
    ...
    "fig11_fp_use_by_education.png", "χ²=9.818, p=0.020, V=0.160",
)
...
fp_prevalence_bar(
    ...
    "fig12_fp_use_by_occupation.png", "χ²=23.853, p<0.001, V=0.249",
)
...
fp_prevalence_bar(
    ...
    "fig13_fp_use_by_husband_reaction.png", "χ²=234.077, p<0.001, V=0.781",
)

fp_prevalence_bar(
    "community_acceptability",
    {ca: ca for ca in ["Not acceptable", "Not sure", "Somewhat acceptable", "Very acceptable"]},
    ["Not acceptable", "Not sure", "Somewhat acceptable", "Very acceptable"],
    f"FP Use by Community Acceptability (N={n})",
    "fig14_fp_use_by_community_acceptability.png",
    "χ²=45.218, p<0.001, V=0.343", rotate=15,
)
...
fp_prevalence_bar(
    ...
    "fig15_fp_use_by_influencer.png", "χ²=49.476, p<0.001, V=0.359", rotate=15,
)

fig, ax = plt.subplots(figsize=(5, 4))
dep_counseling = df.groupby("received_counseling")["fp_use"].mean() * 100
dep_counseling.index = dep_counseling.index.map({0: "No Counseling", 1: "Received Counseling"})
...
for i, (or_val, p, color) in enumerate(zip(ors, p_vals, colors)):
    ...
    ax.text(
        max(ci_upper[i], or_val) + 0.05, i,
        f"AOR={or_val:.3f}, {p_text}", va="center", fontsize=8,
    )
...
ax.boxplot(groups, labels=["Non-Users", "Current Users"],
...
ax.set_title(f"Satisfaction with FP Info by Use Status\nSpearman ρ=0.330, p<0.001")
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
plt.tight_layout()
