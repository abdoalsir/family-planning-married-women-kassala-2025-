...
edu_clean = {"None": "None", "none": "None", "Primary": "Primary",
             "Secondary": "Secondary", "Higher": "Higher"}
df["education"] = df["education"].map(edu_clean)
df["education_code"] = df["education"].map({"None": 1, "Primary": 2, "Secondary": 3, "Higher": 4})

standard_occ = {"Housewife", "Employed", "Student"}
df["occupation"] = df["occupation"].apply(
    lambda x: x.strip() if isinstance(x, str) and x.strip() in standard_occ
...
df["received_counseling"] = df["received_counseling"].str.strip().map({"Yes": 1, "No": 0})
df["experienced_side_effects"] = df["experienced_side_effects"].str.strip().map({"Yes": 1, "No": 0})
...
method_options = {
    "method_condoms": "Condoms", "method_pills": "Pills",
    "method_injectables": "Injectables", "method_iud": "IUD",
...
df["method_other"] = df["fp_method"].apply(
    lambda x: 1 if isinstance(x, str) and any(
        m.strip() not in standard_methods for m in x.split(",")
    ) else 0
)
